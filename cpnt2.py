...
def autoscale(spec, name, namespace, logger, **kwargs):
    ...
    cpu_threshold_high = spec.get('cpuThresholdHigh', 50)  # Default to 50 millicores
    cpu_threshold_low = spec.get('cpuThresholdLow', 25)  # Default to 25 millicores
    min_replicas = spec.get('minReplicas', 1)  # Default to 1 replica
    max_replicas = spec.get('maxReplicas', 5)  # Default to 5 replicas
